      ...
      <div className="section-container">
        ...
        <div className="mt-12 grid gap-8 lg:grid-cols-2">
          {steps.map((step, index) => (
            <motion.article
              key={step.number}
              className="group rounded-[2rem] border border-gray-100 bg-white p-8 shadow-sm transition-all duration-300 hover:shadow-lg hover:border-gray-200"
              initial={{ opacity: 0, y: 18 }}
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true }}
            ...
            >
              <div className="flex items-start justify-between gap-4">
                <div>
                  <p className="text-xs uppercase tracking-[0.15em] text-gray-900 font-bold bg-gray-100 px-2 py-1 inline-block rounded">Step {step.number}</p>
                ...
                <div className="inline-flex items-center gap-2 rounded-full border border-gray-200 bg-white px-3 py-1.5 text-xs font-bold text-gray-900 whitespace-nowrap group-hover:bg-gray-50 transition-colors shadow-sm">
              ...
              <div className="mt-6 border-t border-gray-100 pt-6">
                <p className="text-sm font-bold text-gray-900 uppercase tracking-wide">{t('service.expect')}</p>
        ...
        >
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
              <p className="text-sm uppercase tracking-[0.11em] text-gray-500 font-bold">{t('service.total.title')}</p>
              <p className="mt-1 text-3xl font-bold text-gray-900">{t('service.total.duration')}</p>
            </div>
            <p className="text-gray-600 max-w-xl italic">{t('service.total.note')}</p>
          </div>
        </motion.div>
      </div>
    </section>
