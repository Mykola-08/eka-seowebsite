      ...
      <AnimatePresence>
        {showSuccess && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -20 }}
            className="fixed top-24 right-4 z-50 bg-green-500 text-white px-6 py-4 rounded-2xl shadow-lg flex items-center space-x-3"
          >
          ...
          <div className="grid gap-8 md:grid-cols-2 lg:gap-12 max-w-5xl mx-auto">
            {availableDiscounts.map((discount, index) => (
              <motion.div
                ...
                className="group relative bg-white rounded-3xl p-8 shadow-sm hover:shadow-xl transition duration-300 border border-gray-100 overflow-hidden"
              >
                {/* Background Gradient */}
