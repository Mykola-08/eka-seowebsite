  ...
  const details = (
     <>
       {service.benefitsKeys && service.benefitsKeys.length > 0 && (
         <div className="mb-8">
           <h4 className="text-xl font-semibold tracking-tight text-gray-900 mb-4">{t('services.mainBenefits') || 'Main Benefits'}</h4>
       ...
       <div className="flex items-center justify-between mt-auto p-6 bg-gray-50 border border-gray-100 rounded-2xl">
          <div className="flex flex-col gap-1">
             <span className="text-sm font-medium text-gray-500 uppercase tracking-widest">{t('common.duration') || 'Duration'}</span>
             <div className="flex items-center gap-2">
                 <Clock className="w-5 h-5 text-gray-400" />
                 <span className="text-lg font-semibold text-gray-900">{service.duration ? `${service.duration} ${t('common.minutes') || 'min'}` : `60 ${t('common.minutes') || 'min'}`}</span>
             </div>
          </div>
          <div className="flex flex-col items-end gap-1">
             <span className="text-sm font-medium text-gray-500 uppercase tracking-widest">{t('common.price') || 'Price'}</span>
